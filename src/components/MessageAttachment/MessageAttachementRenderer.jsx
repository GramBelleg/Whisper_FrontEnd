...
const MessageAttachmentRenderer = ({ myMessage }) => {
    ...
    useEffect(() => {}, [dbRef]) // may add objectUrl to dependencies
        ...
        if (parseInt(myMessage.attachmentType) === 2) {
            return (
                <div className='audio-player bg-gray-100 p-4 rounded-lg w-full max-w-sm'>
                    ...
                    <div className='text-sm text-gray-500 mt-2'>{myMessage.attachmentName}</div>
                </div>
            )
        }
    ...
    return <div className='attachment-container'>{renderAttachment()}</div>
}

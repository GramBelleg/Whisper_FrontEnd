...
const EditablePhoneField = ({ initialPhone, onSave, id, fieldName, error, clearError }) => {
    ...
    const handleBlur = (e) => {
        if (tickButtonRef.current && !tickButtonRef.current.contains(e.relatedTarget) && !phoneInputRef.current.contains(e.relatedTarget)) {
            setIsEditing(false)
            clearError(id)
        }
    }
    ...
    return (
        <div className='flex flex-col mb-4 text-left m-4' ref={phoneInputRef}>
            ...
            {isEditing ? (
                ...
                <div className='flex items-center'>
                    <span
                        className={`text-lg mr-2 w-full break-words ${phone ? 'text-light' : 'text-gray-400'}`}

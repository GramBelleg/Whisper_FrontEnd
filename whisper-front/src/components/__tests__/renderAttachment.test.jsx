...
import { downloadLink } from '../../services/mock/mockData'; 
...
describe('MessageAttachmentRenderer', () => {
  ...
  test('renders video attachment correctly', async () => {
    render(<MessageAttachmentRenderer myMessage={myVideoMessage} onUpdateLink={vi.fn()} />);
  ...
  test('renders audio attachment correctly', async () => {
    render(<MessageAttachmentRenderer myMessage={myAudioMessage} onUpdateLink={vi.fn()} />);
    await waitFor(() => {
  ...
  test('renders photo attachment correctly', async () => {
    render(<MessageAttachmentRenderer myMessage={myPhotoMessage} onUpdateLink={vi.fn()} />);
  ...
  test('renders wrong attachment redirection', async () => {
    render(<MessageAttachmentRenderer myMessage={wrongTypeImage} onUpdateLink={vi.fn()} />);

    await waitFor(() => {
        const photoElement = screen.getByTestId('download-link');
        expect(photoElement).toBeInTheDocument();
        expect(photoElement).toHaveAttribute('download', wrongTypeImage.file.name);

    });
  });
  test('renders download attachment correctly', async () => {
    render(<MessageAttachmentRenderer myMessage={myDownloadFileMessage} onUpdateLink={vi.fn()} />);

    await waitFor(() => {
        const downloadElement = screen.getByTestId('download-link');
        expect(downloadElement).toBeInTheDocument();
        expect(downloadElement).toHaveAttribute('download', myDownloadFileMessage.file.name);
